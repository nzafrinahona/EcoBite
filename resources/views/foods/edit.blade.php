<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Edit Food Item</h1>
                <p class="text-sm text-slate-500 mt-0.5">{{ $foodItem->title }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
            ...
            <form action="{{ route('staff.food-items.update', $foodItem) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf @method('PUT')

                <div>
                    <label for="title" class="block text-sm font-medium text-slate-700 mb-1.5">Title <span class="text-red-500">*</span></label>
                    <input id="title" name="title" type="text" value="{{ old('title', $foodItem->title) }}" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full rounded-xl border-slate-200 focus:ring-green-500 focus:border-green-500 text-sm shadow-sm">{{ old('description', $foodItem->description) }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="price" name="price" type="number" step="0.01" min="0.01" value="{{ old('price', $foodItem->price) }}" required
                               class="w-full rounded-xl border-slate-200 focus:ring-green-500 focus:border-green-500 text-sm shadow-sm">
                    ...
                    <div>
                        ...
                        <input id="original_price" name="original_price" type="number" step="0.01" min="0.01" value="{{ old('original_price', $foodItem->original_price) }}"
                               class="w-full rounded-xl border-slate-200 focus:ring-green-500 focus:border-green-500 text-sm shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="quantity" name="quantity" type="number" min="0" value="{{ old('quantity', $foodItem->quantity) }}" required
                               class="w-full rounded-xl border-slate-200 focus:ring-green-500 focus:border-green-500 text-sm shadow-sm">
                    ...
                    <div>
                        ...
                        <input id="expiry_time" name="expiry_time" type="datetime-local"
                               value="{{ old('expiry_time', $foodItem->expiry_time->format('Y-m-d\TH:i')) }}" required
                               class="w-full rounded-xl border-slate-200 focus:ring-green-500 focus:border-green-500 text-sm shadow-sm">
                ...
                <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                    ...
                    <label for="is_active" class="text-sm font-medium text-slate-700">Active — visible to students</label>
                </div>
                ...
                <div>
                    ...
                    @if($foodItem->image)
                        <img src="{{ Storage::url($foodItem->image) }}" alt="{{ $foodItem->title }}"
                             class="w-28 h-28 object-cover rounded-xl mb-3 border border-slate-200">
                    @endif
